...
namespace Farm.Controllers
{
    ...
    public class UtilController : Controller
    {
        ...
        public UtilController(IConfiguration configuration, FarmDbContext FarmDbContext, IUtilityHelper utilityHelper,
        ...
        [HttpGet]
        [Route("GetFarmFieldByFarmerLookup")]
        public async Task<IActionResult> GetFarmFieldByFarmerLookup(int farmerId)
        {
            try
            {
                var results = await FarmDbContext.FarmField
                .Where(d => d.Farmer_Id == farmerId && d.Survey_Nos != null && !(d.Survey_Nos.Equals("0")))
                .Select(s => new
                {
                    Id = s.Id,
                    Name = s.Survey_Nos
        ...
        public async Task<IActionResult> GetMandalByVillage(int villageId)
        {
            try
            {
                var results = await FarmDbContext.Villages
                            .Where(d => d.Id == villageId)
                .Select(s => new
                {
                    mandalId = s.Mandal_Block_Id,
                }).ToListAsync();
                return Ok(results);
            }
            catch (System.Exception ex)
        ...
        public ActionResult GetDashboardInfo()
        {
            try
            {
                SqlConnection con = new SqlConnection(this.FarmDbContext.Database.GetDbConnection().ConnectionString);
                con.Open();
                SqlCommand cmd = new SqlCommand("GetDashboardInfo", con);
                cmd.CommandType = CommandType.StoredProcedure;
                ...
                while (dataReader.Read())
                {
                    ...
                    model.Parameter = dataReader.GetString(0);
                    model.Value = dataReader.GetInt32(1);
                    modelList.Add(model);
                }
                return Ok(modelList);
            }
