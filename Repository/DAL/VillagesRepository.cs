...
namespace Farm.Repositories
{
    public class VillagesRepository : IVillagesRepository
    {
        ...
        public VillagesRepository(FarmDbContext FarmDbContext)
        {
        ...
        {
            return await FarmDbContext.Villages.OrderByDescending(d => d.Id).ToListAsync();
        }

        public async Task<IEnumerable<object>> GetVillagesLookup()
        {
            return await FarmDbContext.Villages
            .Select(s => new
            {
                Id = s.Id,
                Name = s.Name
        ...
        {
            return await FarmDbContext.Villages.FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Villages> CreateVillages(Villages villages)
        {
            villages.Id = GetNextId();
            await FarmDbContext.AddRangeAsync(villages);
            await FarmDbContext.SaveChangesAsync();

            return this.FarmDbContext.Villages.First(a => a.Id == villages.Id);
        }

        public async Task<Villages> UpdateVillages(int Id, Villages villages)
        {
            var villagesDetails = await FarmDbContext.Villages.FirstOrDefaultAsync(x => x.Id == Id);
            ...
            await FarmDbContext.SaveChangesAsync();
            return this.FarmDbContext.Villages.First(a => a.Id == villagesDetails.Id);
        }

	  public async Task<IEnumerable<Villages>> DeleteVillages(int id)
      {
            var list = await FarmDbContext.Villages.Where(s => id == s.Id).ToListAsync();
            FarmDbContext.Villages.RemoveRange(list);
            await FarmDbContext.SaveChangesAsync();
            return list;
      }
	...
	public async Task<Villages> UpdateVillagesStatus(int Id)
        {
            ...
            villagesDetails.IsActive = false;
            await FarmDbContext.SaveChangesAsync();
            return this.FarmDbContext.Villages.First(a => a.Id == villagesDetails.Id);
        }
      ...
      {
        int? maxId = FarmDbContext.Villages.Max(p => p.Id);
        if (maxId == null)
        {
            maxId = 0;
        }
...
public Dictionary<string, object> SearchVillages(string searchString, int pageNumber, int pageSize, string sortColumn, string sortDirection)
        {
