...
namespace Farm.Repositories
{
    public class Users_TypesRepository : IUsers_TypesRepository
    {
        ...
        public Users_TypesRepository(FarmDbContext FarmDbContext)
        {
        ...
        {
            return await FarmDbContext.Users_Types.OrderByDescending(d => d.Id).ToListAsync();
        }

        public async Task<IEnumerable<object>> GetUsers_TypesLookup()
        {
            return await FarmDbContext.Users_Types
            .Select(s => new
            {
                Id = s.Id,
                Name = s.Name
        ...
        {
            return await FarmDbContext.Users_Types.FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Users_Types> CreateUsers_Types(Users_Types users_Types)
        {
            users_Types.Id = GetNextId();
            await FarmDbContext.AddRangeAsync(users_Types);
            await FarmDbContext.SaveChangesAsync();

            return this.FarmDbContext.Users_Types.First(a => a.Id == users_Types.Id);
        }

        public async Task<Users_Types> UpdateUsers_Types(int Id, Users_Types users_Types)
        {
            var users_TypesDetails = await FarmDbContext.Users_Types.FirstOrDefaultAsync(x => x.Id == Id);
            ...
            await FarmDbContext.SaveChangesAsync();
            return this.FarmDbContext.Users_Types.First(a => a.Id == users_TypesDetails.Id);
        }

	  public async Task<IEnumerable<Users_Types>> DeleteUsers_Types(int id)
      {
            var list = await FarmDbContext.Users_Types.Where(s => id == s.Id).ToListAsync();
            FarmDbContext.Users_Types.RemoveRange(list);
            await FarmDbContext.SaveChangesAsync();
            return list;
      }
	...
	public async Task<Users_Types> UpdateUsers_TypesStatus(int Id)
        {
            ...
            users_TypesDetails.IsActive = false;
            await FarmDbContext.SaveChangesAsync();
            return this.FarmDbContext.Users_Types.First(a => a.Id == users_TypesDetails.Id);
        }
      ...
      {
        int? maxId = FarmDbContext.Users_Types.Max(p => p.Id);
        if (maxId == null)
        {
            maxId = 0;
        }
