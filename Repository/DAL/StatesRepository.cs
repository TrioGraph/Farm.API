...
namespace Farm.Repositories
{
    public class StatesRepository : IStatesRepository
    {
        ...
        public StatesRepository(FarmDbContext FarmDbContext)
        {
        ...
        {
            return await FarmDbContext.States.OrderByDescending(d => d.Id).ToListAsync();
        }

        public async Task<IEnumerable<object>> GetStatesLookup()
        {
            return await FarmDbContext.States
            .Select(s => new
            {
                Id = s.Id,
                Name = s.Name
        ...
        {
            return await FarmDbContext.States.FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<States> CreateStates(States states)
        {
            states.Id = GetNextId();
            await FarmDbContext.AddRangeAsync(states);
            await FarmDbContext.SaveChangesAsync();

            return this.FarmDbContext.States.First(a => a.Id == states.Id);
        }

        public async Task<States> UpdateStates(int Id, States states)
        {
            var statesDetails = await FarmDbContext.States.FirstOrDefaultAsync(x => x.Id == Id);
            ...
            statesDetails.Id = Id;
            await FarmDbContext.SaveChangesAsync();
            return this.FarmDbContext.States.First(a => a.Id == statesDetails.Id);
        }

	  public async Task<IEnumerable<States>> DeleteStates(int id)
      {
            var list = await FarmDbContext.States.Where(s => id == s.Id).ToListAsync();
            FarmDbContext.States.RemoveRange(list);
            await FarmDbContext.SaveChangesAsync();
            return list;
      }
	...
	public async Task<States> UpdateStatesStatus(int Id)
      ...
      {
        int? maxId = FarmDbContext.States.Max(p => p.Id);
        if (maxId == null)
        {
            maxId = 0;
        }
...
public Dictionary<string, object> SearchStates(string searchString, int pageNumber, int pageSize, string sortColumn, string sortDirection)
        {
