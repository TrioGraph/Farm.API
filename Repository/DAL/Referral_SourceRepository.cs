...
{
    public class Referral_SourceRepository : IReferral_SourceRepository
    {
        private readonly FarmDbContext FarmDbContext;

        public Referral_SourceRepository(FarmDbContext FarmDbContext)
        {
            this.FarmDbContext = FarmDbContext;
        }

        public async Task<List<Referral_Source>> GetAllReferral_Source()
        {
            return await FarmDbContext.Referral_Source.OrderByDescending(d => d.Id).ToListAsync();
        }

        public async Task<IEnumerable<object>> GetReferral_SourceLookup()
        {
            return await FarmDbContext.Referral_Source
            .Select(s => new
            {
                Id = s.Id,
                Name = s.Name
        ...
        public async Task<Referral_Source> GetReferral_SourceById(int id)
        {
            return await FarmDbContext.Referral_Source.FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Referral_Source> CreateReferral_Source(Referral_Source referral_Source)
        {
            referral_Source.Id = GetNextId();
            await FarmDbContext.AddRangeAsync(referral_Source);
            await FarmDbContext.SaveChangesAsync();

            return this.FarmDbContext.Referral_Source.First(a => a.Id == referral_Source.Id);
        }

        public async Task<Referral_Source> UpdateReferral_Source(int Id, Referral_Source referral_Source)
        {
            var referral_SourceDetails = await FarmDbContext.Referral_Source.FirstOrDefaultAsync(x => x.Id == Id);
            if (referral_SourceDetails == null)
            {
                return null;
            }
            System.Reflection.PropertyInfo[] propertiesInfo = referral_SourceDetails.GetType().GetProperties();
            foreach(System.Reflection.PropertyInfo propInfo in propertiesInfo)
            {
                propInfo.SetValue(referral_SourceDetails, referral_Source.GetType().GetProperty(propInfo.Name).GetValue(referral_Source));
            }
            referral_SourceDetails.Id = Id;
            await FarmDbContext.SaveChangesAsync();
            return this.FarmDbContext.Referral_Source.First(a => a.Id == referral_SourceDetails.Id);
        }

	  public async Task<IEnumerable<Referral_Source>> DeleteReferral_Source(int id)
      {
            var list = await FarmDbContext.Referral_Source.Where(s => id == s.Id).ToListAsync();
            FarmDbContext.Referral_Source.RemoveRange(list);
            await FarmDbContext.SaveChangesAsync();
            return list;
      }

	public async Task<Referral_Source> UpdateReferral_SourceStatus(int Id)
        {
            var referral_SourceDetails = await FarmDbContext.Referral_Source.FirstOrDefaultAsync(x => x.Id == Id);
            if (referral_SourceDetails == null)
            {
            ...
            referral_SourceDetails.IsActive = false;
            await FarmDbContext.SaveChangesAsync();
            return this.FarmDbContext.Referral_Source.First(a => a.Id == referral_SourceDetails.Id);
        }
      ...
      {
        int? maxId = FarmDbContext.Referral_Source.Max(p => p.Id);
        if (maxId == null)
        {
            maxId = 0;
        }
...
public Dictionary<string, object> SearchReferral_Source(int userId, string searchString, int pageNumber, int pageSize, string sortColumn, string sortDirection, bool isColumnSearch = false, string columnName = "", string columnDataType = "", string operatorType = "", string value1 = "", string value2 = "")
        {
            ...
            try
            {
                ...
                adapter.SelectCommand = new SqlCommand("GetSearchReferral_Source", new SqlConnection(ConnectionString));
