...
namespace Farm.Repositories
{
    public class WorkflowRepository : IWorkflowRepository
    {
        ...
        public WorkflowRepository(FarmDbContext FarmDbContext)
        {
        ...
        {
            return await FarmDbContext.Workflow.OrderByDescending(d => d.Id).ToListAsync();
        }

        public async Task<IEnumerable<object>> GetWorkflowLookup()
        {
            return await FarmDbContext.Workflow
            .Select(s => new
            {
                Id = s.Id,
                Name = s.Name
        ...
        {
            return await FarmDbContext.Workflow.FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Workflow> CreateWorkflow(Workflow workflow)
        {
            workflow.Id = GetNextId();
            await FarmDbContext.AddRangeAsync(workflow);
            await FarmDbContext.SaveChangesAsync();

            return this.FarmDbContext.Workflow.First(a => a.Id == workflow.Id);
        }

        public async Task<Workflow> UpdateWorkflow(int Id, Workflow workflow)
        {
            var workflowDetails = await FarmDbContext.Workflow.FirstOrDefaultAsync(x => x.Id == Id);
            if (workflowDetails == null)
            {
                return null;
            }
            System.Reflection.PropertyInfo[] propertiesInfo = workflowDetails.GetType().GetProperties();
            foreach(System.Reflection.PropertyInfo propInfo in propertiesInfo)
            {
                propInfo.SetValue(workflowDetails, workflow.GetType().GetProperty(propInfo.Name).GetValue(workflow));
            }
            workflowDetails.Id = Id;
            await FarmDbContext.SaveChangesAsync();
            return this.FarmDbContext.Workflow.First(a => a.Id == workflowDetails.Id);
        }

	  public async Task<IEnumerable<Workflow>> DeleteWorkflow(int id)
      {
            var list = await FarmDbContext.Workflow.Where(s => id == s.Id).ToListAsync();
            FarmDbContext.Workflow.RemoveRange(list);
            await FarmDbContext.SaveChangesAsync();
            return list;
      }

	public async Task<Workflow> UpdateWorkflowStatus(int Id)
        {
            var workflowDetails = await FarmDbContext.Workflow.FirstOrDefaultAsync(x => x.Id == Id);
            if (workflowDetails == null)
            {
      ...
      {
        int? maxId = FarmDbContext.Workflow.Max(p => p.Id);
        if (maxId == null)
        {
            maxId = 0;
        }
...
public Dictionary<string, object> SearchWorkflow(int userId, string searchString, int pageNumber, int pageSize, string sortColumn, string sortDirection, bool isColumnSearch = false, string columnName = "", string columnDataType = "", string operatorType = "", string value1 = "", string value2 = "")
        {
            ...
            try
            {
                ...
                adapter.SelectCommand = new SqlCommand("GetSearchWorkflow", new SqlConnection(ConnectionString));
