...
namespace Farm.Repositories
{
    public class UsersRepository : IUsersRepository
    {
        ...
        public UsersRepository(FarmDbContext FarmDbContext)
        {
        ...
        {
            return await FarmDbContext.Users.OrderByDescending(d => d.Id).ToListAsync();
        }

        public async Task<IEnumerable<object>> GetUsersLookup()
        {
            return await FarmDbContext.Users
            .Select(s => new
            {
                Id = s.Id,
                Name = s.First_Name
        ...
        {
            return await FarmDbContext.Users.FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Users> CreateUsers(Users users)
        {
            users.Id = GetNextId();
            await FarmDbContext.AddRangeAsync(users);
            await FarmDbContext.SaveChangesAsync();

            return this.FarmDbContext.Users.First(a => a.Id == users.Id);
        }

        public async Task<Users> UpdateUsers(int Id, Users users)
        {
            var usersDetails = await FarmDbContext.Users.FirstOrDefaultAsync(x => x.Id == Id);
            ...
            usersDetails.Id = Id;
            await FarmDbContext.SaveChangesAsync();
            return this.FarmDbContext.Users.First(a => a.Id == usersDetails.Id);
        }

	  public async Task<IEnumerable<Users>> DeleteUsers(int id)
      {
            var list = await FarmDbContext.Users.Where(s => id == s.Id).ToListAsync();
            FarmDbContext.Users.RemoveRange(list);
            await FarmDbContext.SaveChangesAsync();
            return list;
      }
	...
	public async Task<Users> UpdateUsersStatus(int Id)
      ...
      {
        int? maxId = FarmDbContext.Users.Max(p => p.Id);
        if (maxId == null)
        {
            maxId = 0;
        }
