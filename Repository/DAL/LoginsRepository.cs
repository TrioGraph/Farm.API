...
namespace Farm.Repositories
{
    public class LoginsRepository : ILoginsRepository
    {
        ...
        public LoginsRepository(FarmDbContext FarmDbContext)
        {
            this.FarmDbContext = FarmDbContext;
        }

        public async Task<List<Logins>> GetAllLogins()
        {
            return await FarmDbContext.Logins.OrderByDescending(d => d.Id).ToListAsync();
        }

        public async Task<IEnumerable<object>> GetLoginsLookup()
        {
            return await FarmDbContext.Logins
            .Select(s => new
            {
                Id = s.Id,
                Name = s.Login
        ...
        public async Task<Logins> GetLoginsById(int id)
        {
            return await FarmDbContext.Logins.FirstOrDefaultAsync(x => x.Id == id);
        }

        public async Task<Logins> CreateLogins(Logins logins)
        {
            logins.Id = GetNextId();
            await FarmDbContext.AddRangeAsync(logins);
            await FarmDbContext.SaveChangesAsync();

            return this.FarmDbContext.Logins.First(a => a.Id == logins.Id);
        }

        public async Task<Logins> UpdateLogins(int Id, Logins logins)
        {
            var loginsDetails = await FarmDbContext.Logins.FirstOrDefaultAsync(x => x.Id == Id);
            if (loginsDetails == null)
            {
                return null;
            }
            System.Reflection.PropertyInfo[] propertiesInfo = loginsDetails.GetType().GetProperties();
            foreach(System.Reflection.PropertyInfo propInfo in propertiesInfo)
            {
                propInfo.SetValue(loginsDetails, logins.GetType().GetProperty(propInfo.Name).GetValue(logins));
            }
            loginsDetails.Id = Id;
            await FarmDbContext.SaveChangesAsync();
            return this.FarmDbContext.Logins.First(a => a.Id == loginsDetails.Id);
        }

	  public async Task<IEnumerable<Logins>> DeleteLogins(int id)
      {
            var list = await FarmDbContext.Logins.Where(s => id == s.Id).ToListAsync();
            FarmDbContext.Logins.RemoveRange(list);
            await FarmDbContext.SaveChangesAsync();
            return list;
      }

	public async Task<Logins> UpdateLoginsStatus(int Id)
        {
            var loginsDetails = await FarmDbContext.Logins.FirstOrDefaultAsync(x => x.Id == Id);
            if (loginsDetails == null)
            {
      ...
      {
        int? maxId = FarmDbContext.Logins.Max(p => p.Id);
        if (maxId == null)
        {
            maxId = 0;
        }
...
public Dictionary<string, object> SearchLogins(int userId, string searchString, int pageNumber, int pageSize, string sortColumn, string sortDirection, bool isColumnSearch = false, string columnName = "", string columnDataType = "", string operatorType = "", string value1 = "", string value2 = "")
        {
            ...
            try
            {
                ...
                adapter.SelectCommand = new SqlCommand("GetSearchLogins", new SqlConnection(ConnectionString));
